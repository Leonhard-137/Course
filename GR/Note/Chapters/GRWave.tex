...
\end{align}
这就是说，在线性化的形式下，如果需要取谐和坐标，只需要满足线性扰动之逆反的四散度为 0 即可。
...
在选取好所谓的谐和坐标定好度规之后，依旧存在着一些坐标，它们都满足谐和条件但是又不是相同的坐标。类似电动力学，我们先取一个坐标变换的形式，探究坐标变换后同一点的度规变化，度规变化前后都要满足谐和方程，这样就找到所谓的残余规范。
考虑无穷小坐标变换
\begin{align}
x^{\mu}
\longrightarrow
...
\subsection{平面波扰动下的粒子运动}
检验平面引力波对粒子运动的扰动，不能考察其时空距离，因为在TT规范下：
\begin{align}
  \Gamma^{t}{}_{t} &= 0 \\
  \Gamma^{i}{}_{tt} &= 0
\end{align}
...
\begin{align}
  ...
  x^i &= \text{const.}
\end{align}
也就是说，在TT规范下，初始静止的粒子的时空坐标不变，不会受到引力波的影响而运动。我们考察
\begin{align}
  R^2 = x^2 + y^2
\end{align}
这个量不会因为坐标的选取和时空的扰动改变。取引力波传播方向为 $z$，可设：
\begin{align}
  h_{\mu\nu} = A_{\mu\nu}\exp[i(kz - \omega t)] +
\end{align}
考虑某一固定空间点，$h_{\mu\nu}$ 只受时间影响，结合TT规范把 $h_{\mu\nu}$ 分解为：
\begin{align}
  h_{+} &= \begin{pmatrix}
    \cos{\omega r} & 0 \\
    0 & -\cos{\omega r}
...
现在说明实验上如何检测空间的改变，取原始情况下有排列成圆的一组粒子，TT规范选取的坐标系中，点 $x^{i}$ 与原点的空间距离为：
\begin{align}
  L^2 = x^{i}(\delta_{ij}+h_{ij})x^{j}
\end{align}
这里严格来说应该取空间坐标的微分并积分，但是我们在意的空间近乎均匀，所以忽略积分过程。实验中的欧几里得坐标系，将粒子与原点的距离表示为：
\begin{align}
  L = \widetilde{x}^{i}\delta_{ij} \delta_{ij} \widetilde{x}^{j}
\end{align}
通过一个坐标变换可以联系这两个坐标系：
\begin{align}
  \begin{pmatrix}
...
\begin{align}
  ...
  \end{pmatrix}
\end{align}
首先，我们考虑两点之间的空间距离。由于 \textbf{TT 规范} 到 \textbf{欧几里得坐标系} 之间的变换只是一个坐标变换，因此空间距离的实际值不会受到坐标变换的影响。基于这一点，我可以通过坐标变换关系来连接这两个坐标系。另一方面，通过测地线方程可以得出，在 \textbf{TT 规范} 下，$x^2 + y^2$ 是一个定值（这个值并不是实际的物理距离，否则应使用度规来度量），且该定值在坐标变换下不会改变。我们可以通过坐标变换来表达这个定值，最终得到一个二次型方程。

尽管我测量的距离是欧几里得坐标下的坐标，但这些坐标的变化受到了二次型约束的限制。随着时间的推移，这个二次型会像椭圆一样旋转，从而导致椭圆的长短轴交替变化。我的实验测量结果正是这种椭圆形变换的体现。通过测量长短轴的变化，可以推导出引力波对时空的扰动。

LIGO实验就是一个大型的迈克尔逊干涉仪，当引力波经过的时候，原本相等的光程出现了光程差，由此产生干涉效应，我们可以由此得到距离关系。