...
\end{align}
其中 $\xi^\mu(x)$ 为任意 $C^\infty$ 矢量场，称为该流的生成矢量场。把点 $P$ 送到 $Q=\phi_\epsilon(P)$ 的同时，也可视为一次被动坐标变换；在 $\epsilon\!\to\!0$ 的极限下两种观点等价。
\end{definition}
...
为比较同一点的张量值，引入记号 $T(P\!\to\!Q)$：先在变换下把 $T$ 拖到 $Q$，再把其值回比较到 $P$。沿矢量场 $\xi$ 的\emph{李导数}定义为
\begin{align}
\mathcal{L}_\xi T \equiv \lim_{\epsilon\to 0}\frac{T(Q)-T(P\!\to\!Q)}{\epsilon} \nonumber
\end{align}
它描述了张量场在 $\xi$ 生成的流下的一阶变化率。

\paragraph{标量场。}
对标量 $f$，有 $F(P\!\to\!Q)=F(P)=f(P)$，因此
\begin{align}
...
\begin{align}
(\mathcal{L}_\xi k)^\mu
=\lim_{\epsilon\to 0}\frac{k^\mu(Q)-k^\mu(P\!\to\!Q)}{\epsilon}
=\xi^\nu\partial_\nu k^\mu-k^\nu\partial_\nu\xi^\mu .
\tag{1}
\end{align}

\paragraph{协变矢量。}
令 $p_\mu$ 为 $1$-形式。由 Leibniz 相容性（见下）与
...
\begin{derivationnote}[一般张量的分量公式]
